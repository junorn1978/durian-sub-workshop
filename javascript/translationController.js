...
async function pump() {
  ...
  inFlight++;
  try {
    //Logger.debug('[DEBUG] [Translation] 執行佇列任務:', { inFlight, queueLength: queue.length });
    next.resolve(await next.task());
  } catch (e) {
    Logger.error('[ERROR] [Translation] 佇列任務失敗:', { error: e.message });
    next.reject(e);
  } finally {
    inFlight--;
    Logger.debug('[DEBUG] [Translation] 任務完成，繼續處理佇列:', { inFlight, queueLength: queue.length });
    pump();
  }
}
...
async function updateTranslationUI(data, targetLangs, minDisplayTime, sequenceId) {
  ...
  if (stopbutton.disabled) {
    Logger.debug('[DEBUG] [Translation] 停止按鈕啟用，跳過更新 UI');
    return;
  }
  ...
  Logger.debug('[DEBUG] [Translation] updateTranslationUI 開始:', { 
    data: data, 
  ...
  targetLangs.forEach((lang, index) => {
    ...
    if (span && data?.translations && data.translations[index]) {
      ...
      Logger.debug('[DEBUG] [Translation] 推入 displayBuffers:', { 
        target: `target${index + 1}`, 
        text: filteredText, 
        sequenceId: data.sequenceId ?? sequenceId 
      });
    } else {
      Logger.warn('[WARN] [Translation] 無法推入 displayBuffers:', { 
        spanExists: !!span, 
  ...
  if (!bufferCheckInterval) {
    ...
    //Logger.debug('[DEBUG] [Translation] 啟動緩衝區監控，間隔 500ms');
  }
}
...
function processDisplayBuffers() {
  ...
  if (!hasData) {
    if (now - lastLogTime >= LOG_THROTTLE_MS) {
      //Logger.debug('[DEBUG] [Translation] processDisplayBuffers 無數據');
      lastLogTime = now;
    }
    if (bufferCheckInterval) {
      clearInterval(bufferCheckInterval);
      bufferCheckInterval = null;
      //Logger.debug('[DEBUG] [Translation] 停止緩衝區監控，無數據');
    }
    return;
  }
  ...
  ['target1', 'target2', 'target3'].forEach(key => {
    ...
    try {
      ...
      if (currentDisplays[key] && now - currentDisplays[key].startTime < currentDisplays[key].minDisplayTime * 1000) {
        if (now - lastLogTime >= LOG_THROTTLE_MS) {
          Logger.debug('[DEBUG] [Translation] 未達 minDisplayTime，跳過:', { 
            key, 
      ...
      if (nextIndex !== -1) {
        ...
        requestAnimationFrame(() => {
          ...
          //Logger.debug('[DEBUG] [Translation] 更新 target-text:', { target: key, text: next.text, sequenceId: next.sequenceId });
        });
        ...
        Logger.info('[INFO] [Translation] 更新翻譯文字:', { 
          lang: langSelect, 
          text: next.text, 
          sequenceId: next.sequenceId 
        });
        ...
        if (now - lastLogTime >= LOG_THROTTLE_MS) {
          //Logger.debug('[DEBUG] [Translation] 無新結果可顯示:', { key, lastSequenceId });
          lastLogTime = now;
        }
      }
    } catch (error) {
      Logger.error('[ERROR] [Translation] processDisplayBuffers 錯誤:', { key, error: error.message });
    }
  });
}
  ...
  return enqueue(async () => {
    ...
    try {
      ...
      if (currentMode === 'gas') {
        ...
        Logger.debug('[DEBUG] [translationController.js] 模式: GAS, Script ID:', cleanId);
        ...
        Logger.debug('[DEBUG] [translationController.js] 模式: Link, URL:', serviceUrl);
...
window.addEventListener('beforeunload', () => {
  translatorCache.forEach((translator, cacheKey) => {
    translator.destroy();
    Logger.debug('[DEBUG] [Translation] 清理 Translator 物件:', { cacheKey });
  });
  translatorCache.clear();
  Logger.debug('[DEBUG] [Translation] 清理 translatorCache');
  if (bufferCheckInterval) {
    clearInterval(bufferCheckInterval);
    bufferCheckInterval = null;
    Logger.debug('[DEBUG] [Translation] 清理 bufferCheckInterval');
  }
  // 清理佇列
  queue.forEach(task => task.reject(new Error('頁面即將關閉，任務被取消')));
  queue.length = 0;
  Logger.debug('[DEBUG] [Translation] 清理佇列');
});
