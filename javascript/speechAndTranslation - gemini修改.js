...
document.addEventListener("DOMContentLoaded", () => {
  console.info( "[INFO]", "[SpeechAndTranslation] 腳本已載入。");
  // ==========================================================================
  // 常量與配置
  // ==========================================================================
  const CONFIG = {
  ...
  function resetState() {
    ...
    console.info( "[INFO]", "[SpeechAndTranslation] 狀態已重置。");
  }
...
function checkInterimStagnation(sourceLang) {
  ...
  if (
    ...
    console.info( "[INFO]", "[SpeechAndTranslation] 暫存文字停滯，正在發送翻譯：", {
  ...
  function initializeElements() {
    ...
    Object.entries(ELEMENT_IDS).forEach(([key, id]) => {
      ...
      console.debug( "[DEBUG]", "[SpeechAndTranslation] Looking for element:", id, "Found:", !!forwardRef[key]);
    ...
    console.info( "[INFO]", "[SpeechAndTranslation] 所有元素已初始化。");
    return forwardRef;
  }
  ...
  function initializeSpeechRecognition(sourceLanguageSelect) {
    ...
    console.info( "[INFO]", "[SpeechAndTranslation] 已以指定語言初始化語音識別：", recognition.lang);
  ...
  function replacePunctuation(text) {
    ...
    console.debug( "[DEBUG]", "[SpeechAndTranslation] 已替換標點符號：", {
      original: text,
      replaced: result
    });
  ...
  function sendTranslation(text, sourceLang) {
    ...
    console.info( "[INFO]", "[SpeechAndTranslation] 正在發送翻譯：", {
      sequenceNumber,
      text,
      length: text.length
    });
  ...
  function startTranslationTimer(sourceLang) {
    if (state.translationTimer) {
      console.info( "[INFO]", "[SpeechAndTranslation] 翻譯計時器已啟動，正在更新文字：", {
    ...
    console.info( "[INFO]", "[SpeechAndTranslation] 已開始翻譯計時器：", {
      text: state.pendingTranslationText,
      length: state.pendingTranslationText.length
    });
  }
  ...
  function truncateToLastChunk(text) {
    ...
    console.debug( "[DEBUG]", "[SpeechAndTranslation] 正在截斷文字：", {
      sourceLang,
      chunkSize,
  ...
  function applyTextReplacements(text) {
    if (!text || text.trim() === '' || text.trim() === 'っ') {
      console.info( "[INFO]", "[SpeechAndTranslation] 跳過無效文字：", {
        original: text
      });
      return '';
    }
    ...
    if (result !== text) {
      console.info( "[INFO]", "[SpeechAndTranslation] 已套用文字替換：", {
        original: text,
        replaced: result
      });
    }
  ...
  window.SpeechRecognitionAPI = {
    start() {
      console.debug( "[DEBUG]", "[SpeechAndTranslation] 嘗試啟動語音識別，狀態：", state.isRecognitionRunning);
      ...
      texts.target2 = "";
      texts.target3 = "";
      console.info( "[INFO]", "[SpeechAndTranslation] 已將 isManuallyStopped 重置為：", state.isManuallyStopped);
      ...
      try {
        ...
        console.info( "[INFO]", "[SpeechAndTranslation] 語音識別已啟動。");
    ...
    stop() {
      ...
      console.info( "[INFO]", "[SpeechAndTranslation] 語音識別已停止。");
      return true;
    }
  };
  ...
  // 注意：原有的 handleInterimResult, handleFinalResult, handleDisplayResult, handleTranslationResult
  // 已被移除，其邏輯已整合到下方的 recognition.onresult 函式中，以解決顯示不連貫的問題。

  recognition.onresult = function(event) {
    console.info("[INFO]", "[SpeechAndTranslation] 已收到語音結果，數量：", event.results.length, "，起始索引：", event.resultIndex);
  
    let new_interim_text = "";
    let new_final_text_for_translation = "";
    
    try {
      // --- 核心修改：參考 Chrome Demo 的邏輯 ---
      // 從 event.resultIndex 開始遍歷，這是此次事件中第一個新的結果
      for (let i = event.resultIndex; i < event.results.length; ++i) {
        ...
        // 將片段分為「最終」和「臨時」兩類並分別累加
        if (result.isFinal) {
          // 累加本次事件中所有已確認的片段
          new_final_text_for_translation += transcript.trim() + " "; 
        } else {
          // 臨時文字總是覆蓋，因為它代表當前最新的假設
          new_interim_text += transcript; 
        }
      }
      ...
      // 1. 處理最終確認的文字
      if (new_final_text_for_translation.trim()) {
        const processed_final_text = applyTextReplacements(new_final_text_for_translation.trim());
  
        if (processed_final_text) {
          
          // 【修改點】檢查是否需要清空，只在累加模式下生效
          const truncateMode = localStorage.getItem("text-truncate-mode") || "truncate";
          if (truncateMode !== 'truncate' && state.shouldClearNext) {
            state.finalText = ""; // 清空舊的最終文字
            state.shouldClearNext = false; // 重置標記
            console.info( "[INFO]", "[SpeechAndTranslation] 累加模式：已清空上一句最終結果。");
          }

          // 更新 state 中的最終文字總和
          state.finalText += (state.finalText ? " " : "") + processed_final_text;
          
          // 將新的最終文字發送去翻譯
          decideAndTranslate(processed_final_text, elements.sourceLanguageSelect.value);
          
          // 【修改點】設定標記，以便下一次 isFinal 事件來臨時清空
          if (truncateMode !== 'truncate') {
            state.shouldClearNext = true;
          }
        }
        
        // 因為這部分文字已經確認，所以清空當前的臨時文字狀態
        state.interimText = "";
        state.lastInterimUpdateTime = null;
        if (state.interimStagnationTimer) {
          clearTimeout(state.interimStagnationTimer);
          state.interimStagnationTimer = null;
        }
      ...
      // 2. 處理臨時文字
      if (new_interim_text.trim()) {
        state.interimText = new_interim_text.trim(); // 更新 state 中的臨時文字
      ...
      // 3. 更新畫面顯示 (整合了 handleDisplayResult 的邏輯)
      // 顯示的邏輯是：已確認的最終文字(處理後) + 當前的臨時文字(處理後)
      ...
      if (truncateMode === "truncate") {
        // ====================================================================
        // 【截斷模式】或稱「即時字幕模式」
        // 邏輯：只顯示即時的、會自我截斷的臨時文字，不顯示最終結果。
        // ====================================================================

        // 在此模式下，我們只關心 state.interimText
        let display_text = state.interimText;

        if (display_text) {
            // 重要：先進行關鍵字替換，再截斷，可以避免替換後的詞被錯誤切開
            display_text = applyTextReplacements(display_text);
            
            // 對即時文字本身進行截斷，只顯示最新的片段
            // 這會實現您說的「到四十的時候，前面一到四十的字蓋掉，開始從四十一開始顯示」的效果
            display_text = truncateToLastChunk(display_text);

            // 移除標點
            display_text = replacePunctuation(display_text);

            // 根據設定加上 < > 符號
            const textAlignment = localStorage.getItem("text-alignment") || "left";
        ...
        // 最終顯示的文字就只有處理過的臨時文字
        texts.source = display_text || ""; 

      } else {
        // ====================================================================
        // 【累加模式】
        // 邏輯：顯示「所有最終文字」+「當前臨時文字」，且不截斷。
        // ====================================================================

        // 處理最終文字（不呼叫 truncateToLastChunk）
        let final_display_text = state.finalText;
        final_display_text = applyTextReplacements(final_display_text);
        final_display_text = replacePunctuation(final_display_text);

        // 處理臨時文字
        let interim_display_text = state.interimText;
        if (interim_display_text) {
            interim_display_text = applyTextReplacements(interim_display_text);
            interim_display_text = replacePunctuation(interim_display_text);

            const textAlignment = localStorage.getItem("text-alignment") || "left";
            if (textAlignment === "center") {
                interim_display_text = `< ${interim_display_text} >`;
            }
        }
        
        // 組合兩者
        texts.source = (final_display_text + " " + interim_display_text).trim();
      }
      
      // 最後，無論是哪種模式，都統一更新一次畫面
      updateSectionDisplay();
  ...
  recognition.onend = () => {
    console.info( "[INFO]", "[SpeechAndTranslation] 語音識別已結束，isManuallyStopped：", state.isManuallyStopped, "lastStopWasManual:", state.lastStopWasManual, "restartAttempts:", state.restartAttempts, "elapsedTime:", state.startTime ? (Date.now() - state.startTime) / 1000 : "N/A");
    state.isRecognitionRunning = false;
    if (state.lastStopWasManual) {
      ...
      console.info( "[INFO]", "[SpeechAndTranslation] 手動停止，正在更新 UI。");
    }
    else {
      if (state.restartAttempts < CONFIG.MAX_RESTART_ATTEMPTS) {
        console.info( "[INFO]", `[SpeechAndTranslation] 重新啟動嘗試 ${state.restartAttempts + 1}/${CONFIG.MAX_RESTART_ATTEMPTS}`);
  ...
  recognition.onnomatch = (event) => {
    console.info( "[INFO]", "[SpeechAndTranslation] 語音無法辨識（onnomatch）：", {
      results: event.results ? Array.from(event.results).map(r => ({
        transcript: r[0]?.transcript || "empty",
        isFinal: r.isFinal
      })) : "無結果",
      sequenceNumber: state.currentSequenceNumber, // 修正: 使用 currentSequenceNumber
  ...
  function restartRecognition() {
    if (!ensureRecognition()) {
      console.warn("[SpeechAndTranslation] Cannot restart: recognition not supported."); // 修正訊息
      return;
    }
    // 檢查是否正在運行，避免重複啟動 (雖然 onend 已處理，但多一層保護)
    if (state.isRecognitionRunning) {
      console.warn("[SpeechAndTranslation] Cannot restart: recognition is already running.");
      return;
    }
    try {
      recognition.start();
      ...
      state.interimStagnationTimer = setTimeout(() => checkInterimStagnation(elements.sourceLanguageSelect.value), CONFIG.INTERIM_STAGNATION_TIMEOUT);
      console.info( "[INFO]", "[SpeechAndTranslation] 語音識別已重新啟動。");
      state.restartAttempts = 0;
    }
    catch (error) {
  ...
  function decideAndTranslate(text, sourceLang) {
    ...
    if (!text.trim()) {
      console.info( "[INFO]", "[SpeechAndTranslation] 跳過翻譯：文字為空。");
      return;
    }
    // 注意：這裡不再需要 applyTextReplacements，因為傳入的文字在 onresult 中已經處理過
    const translationText = text; 
    state.pendingTranslationText = (state.pendingTranslationText + " " + translationText).trim(); // 累加待翻譯文字
    if (state.pendingTranslationText.length >= CONFIG.MIN_TEXT_LENGTH) {
      if (state.translationTimer) {
        clearTimeout(state.translationTimer);
        state.translationTimer = null;
      }
  ...
  async function translateText(text, sourceLang, sequenceNumber) {
    ...
    if (!targetLangs.length) {
      console.info( "[INFO]", "[SpeechAndTranslation] 未選擇目標語言。");
      // 注意: 原本的 updateSectionDisplay() 在這裡可能會造成不必要的刷新
      return;
    }
    console.debug( "[DEBUG]", "[SpeechAndTranslation] 正在發送翻譯請求：", {
      sequenceNumber,
      text,
      targetLangs
    });
    const timeoutId = setTimeout(() => {
      if (sequenceNumber === state.expectedSequenceNumber) {
        console.info( "[INFO]", `[SpeechAndTranslation] 翻譯逾時（序列： ${sequenceNumber})`);
    ...
    try {
      ...
      if (apiMode === 'openai') {
        ...
        console.info( "[INFO]", "[SpeechAndTranslation] Gemini API 尚未實作：", { sequenceNumber, text });
        ...
        console.info( "[INFO]", "[SpeechAndTranslation] 已接收翻譯：", {
          sequenceNumber,
    ...
    try {
      ...
      try {
        ...
        content = content
          ...
          .replace(/^`+|\n`+$/g, '')
          .trim();
        console.debug( "[DEBUG]", "[SpeechAndTranslation] 清理後的 OpenAI 回應內容：", { content });
        translations = JSON.parse(content);
        if (!Array.isArray(translations)) {
          throw new Error("回應不是有效的 JSON 陣列。");
        }
      } catch (e) {
        console.info( "[INFO]", "[SpeechAndTranslation] 回應解析失敗，原始內容：", {
          rawContent: data.choices[0]?.message.content
        });
        throw new Error(`無效的 OpenAI 回應格式：${e.message}`);
      }
  
      console.info( "[INFO]", "[SpeechAndTranslation] 已接收 OpenAI 翻譯：", {
        sequenceNumber,
        translations,
        usage: data.usage
      });
  ...
  function handleTranslationResponse(sequenceNumber, text, translations, errorMessage) {
    if (state.ignoreTranslations) {
      console.info( "[INFO]", "[SpeechAndTranslation] 手動停止後忽略翻譯回應：", {
        sequenceNumber
      });
      return;
    }
    if (sequenceNumber < state.expectedSequenceNumber) {
      console.info("[INFO]", "[SpeechAndTranslation] 回傳序號異常", { 
	   sequenceNumber, expectedSequenceNumber: state.expectedSequenceNumber
      });
      return;
    }
    if (sequenceNumber === state.expectedSequenceNumber) {
  ...
  // 事件綁定
  // ==========================================================================
  console.info( "[INFO]", "[SpeechAndTranslation] 正在綁定事件監聽器。");
  elements.startSpeechButton.addEventListener("click", () => {
    console.info( "[INFO]", "[SpeechAndTranslation] 點擊開始按鈕。");
  ...
  elements.stopSpeechButton.addEventListener("click", () => {
    console.info( "[INFO]", "[SpeechAndTranslation] 點擊停止按鈕。");
  ...
  elements.sourceLanguageSelect.addEventListener("change", () => {
    recognition.lang = elements.sourceLanguageSelect.value;
    console.info( "[INFO]", "[SpeechAndTranslation] 已將來源語言更新為：", recognition.lang);
  });
  [elements.targetLanguage1Select, elements.targetLanguage2Select, elements.targetLanguage3Select].forEach((select, index) => {
    select.addEventListener("change", () => {
      ...
      if (select.value === "none") {
        texts[langKey] = "";
        updateSectionDisplay();
        console.info( "[INFO]", `[SpeechAndTranslation] 由於選擇 'none'，已清除 ${langKey}。`);
      }
    });
  });
});