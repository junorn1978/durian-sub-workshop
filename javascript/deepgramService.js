...
socket.onmessage = (message) => {
      try {
        ...
        const alternative = received.channel.alternatives?.[0]; // 提取出來方便存取
        ...
        if (transcript && transcript.trim().length > 0) {
          ...
          // [新增] 解析 Speaker 邏輯
          // 參考你提供的 snippet，我們從 words 陣列中找出 speaker
          let currentSpeaker = null;
          if (alternative.words && alternative.words.length > 0) {
             // 找出這一小段文字中出現的所有 speaker
             const speakersInChunk = [...new Set(alternative.words.map(w => w.speaker).filter(s => s !== undefined))];
             // 簡單策略：取這段文字的第一個主要說話者
             if (speakersInChunk.length > 0) {
                 currentSpeaker = speakersInChunk[0];
             }
          }
          ...
          if (received.is_final) {
            ...
            // [修改] 這裡的 onTranscriptUpdate 增加第四個參數 currentSpeaker
            if (finalResultCount >= 1 && (hasPunctuation || sentenceBuffer.trim().length >= langObj.chunkSize * 2)) {
              if (onTranscriptUpdate) {
                // 傳遞 speaker 資訊
                onTranscriptUpdate(sentenceBuffer, true, true, currentSpeaker); 
              }
              resetSentenceBuffer();
              ...
              if (onTranscriptUpdate) {
                  // 傳遞 speaker 資訊
                  onTranscriptUpdate(sentenceBuffer, false, false, currentSpeaker); 
            ...
            if (onTranscriptUpdate) { 
                // [修改] 傳遞 speaker 資訊
                onTranscriptUpdate(sentenceBuffer + transcript, false, false, currentSpeaker); 
            }
